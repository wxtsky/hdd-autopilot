...
use crate::model::{AuthCache, AuthConfig, LIGHTSOUT_DIFFICULTY_ORDER, LightsoutSession};
...
pub fn run_batch(
    ...
    let state = Arc::new(Mutex::new(BatchState {
        ...
        result_log_dir: resolve_data_file_path("log/lightsout"),
        log: log.clone(),
    }));
    let accounts = config.accounts.clone();
    let base_url = config.base_url.clone();
    log.line_fmt(format_args!("开始自动点灯，本次会处理 {} 个账号。", accounts.len()));
    ...
    for h in handles {
        match h.join() {
            ...
            Err(_) => state.lock().unwrap().log.line("自动点灯任务异常退出。"),
        }
    }
    print_account_reward_summary(log, "自动点灯", &reward_summaries);
...
fn run_account(
    ...
    if let Some(active) = me.active_session.clone() {
        if active.status == "pending" || active.status.is_empty() {
            ...
            let r = solve_and_play(cancel_flag, state, runtime, active, interval)?;
            total += r;
        }
    }

    for difficulty in LIGHTSOUT_DIFFICULTY_ORDER.iter() {
        ui::check_cancel(cancel_flag)?;
        let me = with_auth_retry_api_until_success(
            cancel_flag,
            state,
        ...
        for i in 0..remaining {
            ...
            let start = with_auth_retry_api_until_success(
                ...
                runtime,
                "lightsout start",
                |c, t| c.start_lightsout(t, difficulty),
            )?;
...
fn solve_and_play(
    ...
    let clicks = match crate::solver::lightsout::solve(&session) {
        Some(c) => c,
        None => {
            state.lock().unwrap().log.line_fmt(format_args!(
                "账号 {} 点灯求解失败：对局 {} 无解，放弃这局。",
                runtime.email(),
                session.session_id,
            ));
            ...
            let _ = with_auth_retry_api_until_success(
                ...
                runtime,
                "lightsout abandon",
                |c, t| c.abandon_simple(t, crate::api::LIGHTSOUT_ABANDON_PATH, "/lightsout", sid),
            );
            return Ok(0.0);
        }
    };
    ...
    for (r, c) in clicks {
        ...
        let resp = with_auth_retry_api_until_success(
            ...
            runtime,
            "lightsout click",
            |client, token| client.click_lightsout(token, session_id, r, c),
        )?;
